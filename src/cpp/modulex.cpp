...
void Modulex::init(Napi::Env env, Napi::Object exports) {
	...
	exports.DefineProperties({ SF(moduleLoad), SF(moduleLoadData), SF(moduleRuntimeCompile) });
...
Modulex::Modulex(const Napi::CallbackInfo &info) {
	NAPI_ENV;
	...
	if (info.Length() == 1) {
		...
		const char *opts[] = {
			"--gpu-architecture=compute_75", "--maxrregcount=0", "--std=c++11", "--use_fast_math"
		};
...
JS_IMPLEMENT_METHOD(Modulex, destroy) {
	THIS_CHECK;
	_destroy();
	RET_UNDEFINED;
}
...
JS_IMPLEMENT_GETTER(Modulex, isDestroyed) {
	THIS_CHECK;
	RET_BOOL(_isDestroyed);
}


JS_METHOD(Modulex::moduleLoad) {
	NAPI_ENV;
...
JS_METHOD(Modulex::moduleLoadData) {
	NAPI_ENV;
...
JS_METHOD(Modulex::moduleRuntimeCompile) {
	NAPI_ENV;
	...
	RET_VALUE(_ctorEs5.Value().New(info.Length() > 2 ? 3 : 2, argv));
}

JS_IMPLEMENT_METHOD(Modulex, getFunction) {
	THIS_CHECK;
