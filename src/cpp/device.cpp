...
JS_IMPLEMENT_GETTER(Device, name) {
	THIS_CHECK;
	char deviceName[256];
...
JS_IMPLEMENT_GETTER(Device, totalMem) {
	THIS_CHECK;
...
JS_IMPLEMENT_GETTER(Device, computeCapability) {
	THIS_CHECK;
	int major = 0;
	int minor = 0;
...
JS_IMPLEMENT_GETTER(Device, clockrateMultiprocessorCount) {
	THIS_CHECK;
	int major = 0;
	int minor = 0;
...
JS_IMPLEMENT_METHOD(Device, destroy) {
	THIS_CHECK;
	_destroy();
	RET_UNDEFINED;
}

JS_IMPLEMENT_GETTER(Device, isDestroyed) {
	THIS_CHECK;
	RET_BOOL(_isDestroyed);
}

JS_IMPLEMENT_METHOD(Device, limitMallocHeapSize) {
	THIS_CHECK;
