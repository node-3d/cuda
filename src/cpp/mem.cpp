...
Mem::Mem(const Napi::CallbackInfo &info) {
	NAPI_ENV;
	...
	if (info.Length() == 1) {
		...
		CUresult error = cuMemAllocPitch(&m_devicePtr, &pPitch, widthInBytes, height, elementSizeBytes);
...
JS_IMPLEMENT_GETTER(Mem, devicePtr) {
	THIS_CHECK;
	RET_NUM(static_cast<size_t>(m_devicePtr));
}

JS_IMPLEMENT_METHOD(Mem, destroy) {
	THIS_CHECK;
	_destroy();
	RET_UNDEFINED;
}

JS_IMPLEMENT_GETTER(Mem, isDestroyed) {
	THIS_CHECK;
	RET_BOOL(_isDestroyed);
}

JS_METHOD(Mem::memVBO) {
	NAPI_ENV;
	RET_VALUE(_ctorEs5.Value().New({}));
}

JS_METHOD(Mem::memAlloc) {
	NAPI_ENV;
...
JS_METHOD(Mem::thrust_inclusiveScan) {
	NAPI_ENV;
...
JS_METHOD(Mem::thrust_reduce_floatSum) {
	NAPI_ENV;
...
JS_METHOD(Mem::thrust_reduce_floatMax) {
	NAPI_ENV;
	...
	Napi::Array ra = JS_ARRAY;
	ra.Set(0U, rv.pos);
	ra.Set(1U, rv.val);
	RET_VALUE(ra);
}

JS_METHOD(Mem::thrust_reduce_floatMin) {
	NAPI_ENV;
...
JS_METHOD(Mem::thrust_floatSort_int) {
	NAPI_ENV;
...
JS_METHOD(Mem::thrust_remove_int) {
	NAPI_ENV;
...
JS_IMPLEMENT_METHOD(Mem, copyHtoD) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Mem, copyDtoH) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Mem, copyDtoH_ByPtr) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Mem, copyHtoD_A) {
	THIS_CHECK;
	RET_VALUE(__i_copyHtoD(info));
}

JS_IMPLEMENT_METHOD(Mem, copyDtoH_A) {
	THIS_CHECK;
	RET_VALUE(__i_copyDtoH(info));
}

JS_IMPLEMENT_METHOD(Mem, cudaMemset) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Mem, initVBO) {
	THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Mem, regVBO) {
	THIS_CHECK;
	REQ_UINT32_ARG(0, vboId);
...
JS_IMPLEMENT_METHOD(Mem, unregVBO) {
	THIS_CHECK;
	cuda_vbo_unreg(&vbo);
	RET_UNDEFINED;
}
