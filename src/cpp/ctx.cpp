...
Ctx::Ctx(const Napi::CallbackInfo &info):
_asyncCtx(info.Env(), "Ctx") {
...
JS_IMPLEMENT_GETTER(Ctx, apiVersion) { THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Ctx, destroy) { THIS_CHECK;
	CUresult res = _destroy();
	RET_NUM(res);
}


JS_IMPLEMENT_GETTER(Ctx, isDestroyed) { THIS_CHECK;
	RET_BOOL(_isDestroyed);
}


JS_IMPLEMENT_METHOD(Ctx, pushCurrent) { THIS_CHECK;
	CUresult error = cuCtxPushCurrent(m_context);
	RET_NUM(error);
}

JS_IMPLEMENT_METHOD(Ctx, popCurrent) { THIS_CHECK;
	CUresult error = cuCtxPopCurrent(&m_context);
	RET_NUM(error);
}

JS_IMPLEMENT_METHOD(Ctx, setCurrent) { THIS_CHECK;
	CUresult error = cuCtxSetCurrent(m_context);
	RET_NUM(error);
}

JS_IMPLEMENT_METHOD(Ctx, getCurrent) { THIS_CHECK;
...
JS_IMPLEMENT_METHOD(Ctx, synchronize) { THIS_CHECK;
	...
	if (info.Length() >= 1 && info[0].IsFunction()) {
		...
		params->ctx = this;
		params->cb = Napi::ThreadSafeFunction::New(
			env,
